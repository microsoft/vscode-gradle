    ...
    describe("Without a multi-root workspace", () => {
        ...
        describe("With no pinned tasks", () => {
            it('should build a "No Tasks" tree item when no pinned have been added', async () => {
                ...
                assert.strictEqual(childTreeItem.description, "No pinned tasks");
            });
        });
        ...
        describe("With pinned tasks", () => {
            ...
            it("should build a pinned task treeitem", async () => {
                ...
                assert.strictEqual(pinnedTaskTreeItem.contextValue, TREE_ITEM_STATE_TASK_IDLE);
                assert.strictEqual(pinnedTaskTreeItem.description, "");
                assert.strictEqual(pinnedTaskTreeItem.label, mockTaskDefinition1.script);
                assert.strictEqual(pinnedTaskTreeItem.tooltip, mockTaskDefinition1.description);
                assert.strictEqual(
            ...
            it("should build a pinned task treeitem with args", async () => {
                ...
                await new PinTaskWithArgsCommand(pinnedTasksTreeDataProvider).run(taskItem);
                const children = await pinnedTasksTreeDataProvider.getChildren();
                assert.strictEqual(children.length, 1);
                const pinnedTaskTreeItem = children[0];
                assert.strictEqual(pinnedTaskTreeItem.collapsibleState, vscode.TreeItemCollapsibleState.None);
                assert.strictEqual(pinnedTaskTreeItem.contextValue, TREE_ITEM_STATE_TASK_IDLE + "WithArgs");
                assert.strictEqual(pinnedTaskTreeItem.description, "");
                assert.strictEqual(pinnedTaskTreeItem.label, `${mockTaskDefinition1.script} --info`);
                assert.strictEqual(pinnedTaskTreeItem.tooltip, `(args: --info) ${mockTaskDefinition1.description}`);
                assert.strictEqual(
                    (pinnedTaskTreeItem as PinnedTaskTreeItem).task.definition.id,
                    mockTaskDefinition1.id
                );
            ...
            it("should build a pinned task treeitem with args when pinning an existing pinned task", async () => {
                ...
                sinon.stub(vscode.window, "showInputBox").resolves("--info "); // intentional trailing space
                await new PinTaskWithArgsCommand(pinnedTasksTreeDataProvider).run(pinnedTask as PinnedTaskTreeItem);

                const childrenAfter = await pinnedTasksTreeDataProvider.getChildren();
                assert.strictEqual(childrenAfter.length, 2);
                assert.ok(childrenAfter[0] instanceof PinnedTaskTreeItem, "Pinned task is not PinnedTaskTreeItem");
                assert.ok(childrenAfter[1] instanceof PinnedTaskTreeItem, "Pinned task is not PinnedTaskTreeItem");
                assert.strictEqual(
                ...
                assert.strictEqual(pinnedTaskWithArgsTreeItem.contextValue, TREE_ITEM_STATE_TASK_IDLE + "WithArgs");
                assert.strictEqual(pinnedTaskWithArgsTreeItem.description, "");
                assert.strictEqual(pinnedTaskWithArgsTreeItem.label, `${mockTaskDefinition1.script} --info`);
                assert.strictEqual(
            ...
            it("should remove a pinned task", async () => {
                ...
                assert.strictEqual(childrenBefore.length, 1);
                const pinnedTask = childrenBefore[0];
                assert.ok(pinnedTask instanceof PinnedTaskTreeItem, "Pinned task is not PinnedTaskTreeItem");
                await new RemovePinnedTaskCommand(pinnedTasksTreeDataProvider).run(pinnedTask);
                const childrenAfter = await pinnedTasksTreeDataProvider.getChildren();
                assert.strictEqual(childrenAfter.length, 1);
                const noPinnedTasks = childrenAfter[0];
                assert.ok(noPinnedTasks instanceof NoPinnedTasksTreeItem, "Pinned task is not NoPinnedTasksTreeItem");
            });

            it("should remove all pinned tasks", async () => {
                await new PinTaskCommand(pinnedTasksTreeDataProvider).run(gradleTasks[0]);
                await new PinTaskCommand(pinnedTasksTreeDataProvider).run(gradleTasks[1]);
                const childrenBefore = await pinnedTasksTreeDataProvider.getChildren();
                assert.strictEqual(childrenBefore.length, 2);
                assert.ok(childrenBefore[0] instanceof PinnedTaskTreeItem, "Pinned task is not PinnedTaskTreeItem");
                assert.ok(childrenBefore[1] instanceof PinnedTaskTreeItem, "Pinned task is not PinnedTaskTreeItem");
    ...
    describe("With a multi-root workspace", () => {
        ...
        describe("With pinned tasks", () => {
            it("should build a pinned task treeitem", async () => {
                ...
                assert.strictEqual(pinnedTasksRootProjectTreeItem.parentTreeItem, undefined);
